...
def ai_process_word_advanced(full_text):
    ...
    1. 'process': Use this for methodologies, step-by-step procedures, timelines, or workflows. (Draws connected process cards)
    2. 'table': Use this for comparative matrices, data, lists of items with attributes, or structured parameters.
    3. 'comparison': Use this for contrasting two ideas, Pros vs Cons, Before vs After, or Difference analyses. (Side-by-side)
    4. 'bullets': Use only for simple conceptual summaries or key take-aways.
...
def draw_visuals_on_slide(slide, slide_data):
    ...
    if layout_type == "process" and "process" in content:
        steps = content["process"][:4]  # Maximum 4 steps for optimal design spacing
        num_steps = len(steps)
        if num_steps > 0:
            ...
            for idx, step_data in enumerate(steps):
                ...
                # Add connecting arrow if there's a next step
                if idx < num_steps - 1:
    ...
    # LAYOUT 2: ACADEMIC TABLES
        ...
        num_rows = len(rows) + 1 # Adding header row
            ...
            for row_idx, row_values in enumerate(rows):
                for col_idx, val in enumerate(row_values):
                    if col_idx < num_cols:
                        ...
                        cell.text = str(val)
                        cell.fill.solid()
                        # Alternating background for modern visual feel
                        if row_idx % 2 == 0:
    ...
    # LAYOUT 3: SIDE-BY-SIDE COMPARISON (Columns)
        ...
        p_lt.text = left_title.upper()
        p_lt.font.bold = True
        p_lt.font.size = Pt(16)
        p_lt.font.color.rgb = THEME_DARK_BLUE
        ...
        p_rt.text = right_title.upper()
        p_rt.font.bold = True
        p_rt.font.size = Pt(16)
        p_rt.font.color.rgb = THEME_DARK_BLUE
